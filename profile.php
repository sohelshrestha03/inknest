...
mysqli_stmt_bind_param($userSql, "i", $userId);
...
if ( $_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["update_profile"])) {
    ...
    if ($error === "") {
        ...
        if ($emailChanged || $phoneChanged) {
            ...
            unset(
                $_SESSION["pending_delete_account"],
                ...
                if (!isset($allowedTypes[$mimeType])) {
                    ...
                    if (move_uploaded_file($file["tmp_name"],$destination)) {
                        ...
                        if ($checkProfile) {
                            ...
                            $profileExists = mysqli_stmt_fetch($checkProfile);
                            mysqli_stmt_close(
                                $checkProfile
                            );
                        }
                        ...
                        if ($dbSuccess) {
                            if ($oldPicture !== "") {
                                $oldFile =$uploadDirectory .basename($oldPicture);
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["delete_account"])) {
    ...
    if ($deletePassword === "") {
        ...
        if (!$passwordCheckSql) {
            ...
            if (!mysqli_stmt_execute($passwordCheckSql)) {
                ...
                if (!mysqli_stmt_fetch($passwordCheckSql)) {
                    $storedDeletePassword = "";
                }
                mysqli_stmt_close($passwordCheckSql);
                if (!password_verify($deletePassword,$storedDeletePassword)) {
                    $error ="Current password is incorrect.";
                } else {
                    $_SESSION["pending_delete_account"] = true;
                    $_SESSION["profile_otp_action"] ="delete_account";
                    $_SESSION["profile_otp_email"] =strtolower($email);
                    $_SESSION["profile_otp_purpose"] = "delete_account";
                    unset(
                        $_SESSION["pending_password_hash"],
                        $_SESSION["pending_profile_update"],
                        $_SESSION["otp_verified"],
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["complete_otp_update"])) {
    ...
    $sessionOtpPurpose=$_SESSION["profile_otp_purpose"] ?? "";
    $verifiedOtpPurpose=$_SESSION["otp_verified_purpose"] ?? "";
    $verifiedUserId =
        isset($_SESSION["otp_verified_user_id"])
            ? (int)$_SESSION["otp_verified_user_id"]
            : 0;
    $otpIsVerified =
        isset(
            $_SESSION["otp_verified"],
        ...
        $sessionOtpPurpose !== "" &&
        $verifiedOtpPurpose === $sessionOtpPurpose &&
        (
            $verifiedUserId === 0 ||
    ...
    if (!$otpIsVerified) {
        ...
        if ($action === "change_password") {
            ...
            if (!$pending) {
                ...
                $pendingFirstName =trim($pending["first_name"] ?? "");
                $pendingLastName =trim($pending["last_name"] ?? "");
                $pendingUsername =trim($pending["username"] ?? "");
            ...
            if (!isset($_SESSION["pending_delete_account"]) || $_SESSION["pending_delete_account"] !== true) {
                $error ="Account deletion session expired. Please try again.";
            } else {
                $pictureToDelete = "";
                ...
                try {
                    ...
                    mysqli_stmt_close($otpDeleteSql);
...
$otpAction=$_SESSION["profile_otp_action"] ?? "";
$showPasswordOtp=$otpAction === "change_password" &&
    (
        $sendOtpNow ||
        !isset($_SESSION["otp_verified"])
    );
$showAccountOtp =$otpAction === "account_update" &&
    (
        $sendOtpNow ||
        !isset($_SESSION["otp_verified"])
    );
$showDeleteOtp =
    $otpAction === "delete_account" &&
    isset($_SESSION["pending_delete_account"]) &&
    $_SESSION["pending_delete_account"] === true &&
    (
        $sendOtpNow ||
        !isset($_SESSION["otp_verified"])
...
<main class="container">
    ...
    <div class="profile-grid">
        <div class="card picture-section">
            ...
            <?php if ($profilePicture !== ""): ?>
                <form method="POST">
                    <button type="submit" name="delete_picture" class="btn btn-danger" onclick="return confirm('Are you sure you want to delete your profile picture?');">
                        Delete Picture
                    </button>
                </form>
            <?php endif; ?>
        ...
        <div class="card">
            ...
            <?php if ($showPasswordOtp || $showAccountOtp): ?>
                <hr class="section-divider">
                <h2>Verify OTP</h2>
                <p class="password-note">A verification code has been sent to your current email address.</p>
                <div class="form-group">
                    <label for="profile_otp">Verification Code</label>
                    <input type="text" id="profile_otp" maxlength="6" inputmode="numeric" autocomplete="one-time-code" placeholder="Enter 6-digit OTP">
                </div>
                <button type="button" id="verifyProfileOtp" class="btn btn-primary">Verify OTP</button>
                <button type="button" id="resendProfileOtp" class="btn btn-primary" style="margin-top:10px;">Resend OTP</button>
    ...
    <div class="delete-account-section">
        ...
        <?php else: ?>
            <p class="password-note">Your current password was verified. Enter the OTP sent to your email to permanently delete your account.</p>
            <div class="form-group">
                <label for="profile_otp">Verification Code</label>
                <input type="text" id="profile_otp" maxlength="6" inputmode="numeric" autocomplete="one-time-code" placeholder="Enter 6-digit OTP">
            </div>
            <button type="button" id="verifyProfileOtp" class="btn btn-danger">Verify OTP</button>
            <button type="button" id="resendProfileOtp" class="btn btn-primary" style="margin-top:10px;">Resend OTP</button>
            <p id="profileOtpMessage"class="password-note"></p>
            <form id="completeOtpForm" method="POST" style="display:none;">
                <input type="hidden" name="complete_otp_update" value="1">
    ...
    deleteAccountForm.addEventListener(
        ...
        function (event) {
            const confirmed =confirm(
                    "Are you sure you want to delete your account? You will need to verify an OTP before your account is permanently deleted."
                );
            if (!confirmed) {
                event.preventDefault();
            }
...
function setProfileOtpMessage(
    ...
    profileOtpMessage.style.color =isError ? "#c62828" : "#555";
}
    ...
    verifyProfileOtp.addEventListener(
        ...
        async function () {
            ...
            try {
                ...
                if (profileOtpPurpose === "delete_account") {
                    setProfileOtpMessage(
                        "OTP verified. Deleting your account..."
                    );
                } else {
                    setProfileOtpMessage(
