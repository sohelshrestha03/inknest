...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["final_register"])) {
    if (!isset($_SESSION["register_otp_verified"]) || $_SESSION["register_otp_verified"] !== true) {
        redirectWithAlert(
    ...
    $first = trim($pendingRegistration["fname"] ?? "");
    $last = trim($pendingRegistration["lname"] ?? "");
    $userName = trim($pendingRegistration["uname"] ?? "");
    $email = strtolower(trim($pendingRegistration["email"] ?? ""));
    $phone = trim($pendingRegistration["contact"] ?? "");
    $passwordHash =$pendingRegistration["password_hash"] ?? "";
    $verifiedEmail = strtolower(trim($_SESSION["register_email"] ?? ""));
    ...
    if (mysqli_stmt_fetch($check)) {
        mysqli_stmt_close($check);
        if (strtolower($existingEmail) === $email) {
            unset(
    ...
    $dbError = mysqli_stmt_error($insert);
    $dbErrorNo = mysqli_stmt_errno($insert);
    mysqli_stmt_close($insert);
    error_log(
    ...
    if ($errorMessage === "") {
        ...
        if (!$check) {
            ...
            if (!mysqli_stmt_execute($check)) {
                ...
                if (mysqli_stmt_fetch($check)) {
                    if (strtolower($existingEmail) ===$email) {
                        $errorMessage ="This email address is already registered. Please use another email.";
                    } elseif ($existingUsername ===$userName) {
                        $errorMessage ="Username is already used. Please choose another username.";
                    } elseif ($existingPhone ===$phone) {
                        $errorMessage ="Phone number is already used. Please use another phone number.";
                    }
                }
                mysqli_stmt_close($check);
            }
    ...
    <div class="register-card">
        ...
        <form id="registerForm" action="register.php" method="post">
            <div class="data">
                ...
                <input type="text" id="fname" name="fname" placeholder="Enter your first name" autocomplete="off" value="<?php
                        echo htmlspecialchars(
                            $formFirst,
                            ENT_QUOTES,
                            "UTF-8"
            ...
            <div class="data">
                ...
                <input type="text" id="lname" name="lname" placeholder="Enter your last name" autocomplete="off" value="<?php
                        echo htmlspecialchars(
                            $formLast,
                            ENT_QUOTES,
                            "UTF-8"
            ...
            <div class="data">
                ...
                <input type="text" id="uname" name="uname" placeholder="Enter your username" autocomplete="off" value="<?php
                        echo htmlspecialchars(
                            $formUsername,
                            ENT_QUOTES,
                            "UTF-8"
            ...
            <div class="data">
                ...
                <input type="email" id="email" name="email" placeholder="Enter your email" autocomplete="off" value="<?php
                        echo htmlspecialchars(
                            $formEmail,
                            ENT_QUOTES,
                            "UTF-8"
            ...
            <div class="data">
                ...
                <input type="text" id="contact" name="contact" placeholder="Enter phone number" maxlength="10" autocomplete="off" value="<?php
                        echo htmlspecialchars(
                            $formPhone,
                            ENT_QUOTES,
                            "UTF-8"
            ...
            <div class="data">
                ...
                <input type="password" id="npassword" name="npassword" placeholder="Create a password" minlength="8" required>
            </div>

            <div class="data">
                <label for="cpassword">Confirm Password</label>
                <input type="password" id="cpassword" name="cpassword" placeholder="Confirm your password" minlength="8" required>
            </div>

            <?php if (!$otpStep): ?>
                <div class="buttons">
                    <button type="submit" name="send_otp" value="1" id="sendOtpButton">
                        Register
                    </button>
                    <button type="reset" class="cancel" id="cancelButton">
                        Cancel
                    </button>
                </div>
            <?php endif; ?>
...
document.addEventListener("DOMContentLoaded",function () {
        ...
        const email =(otpSection.dataset.email || "").trim();
        ...
        async function sendOtp() {
            if (!email) {
                showMessage("Email address is missing.",true);
                return;
            }
        ...
        verifyOtpButton.addEventListener(
            ...
            async function () {
                ...
                try {
                    ...
                    showMessage("Email verified. Creating your account...");
                    ...
                    finalInput.name ="final_register";
                    finalInput.value ="1";
...
if (cancelButton) {
    cancelButton.addEventListener(
        "click",
        function () {
            fetch("auth/cancel_registration.php", {
                method: "POST"
            }).catch(function () {
                console.error(
